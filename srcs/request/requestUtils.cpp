...
void copyToRoute(Route &route, std::map<std::string, Route>::const_iterator &it) {
    ...
    for (std::set<std::string>::const_iterator it_methods = route.getAllowedMethods().begin(); it_methods != route.getAllowedMethods().end(); ++it_methods) {
        std::cout << "Allowed Method: " << *it_methods << std::endl;
    }
    // exit(1);
...
void directoryHasIndexFiles(HTTPRequest &request, Route &route, std::vector<std::string> index_files) {
    ...
    for(size_t i = 0; i < index_files.size(); i++) {
        ...
        std::cout << "PATH DZEB:  " << path << std::endl;
        // exit(1);
